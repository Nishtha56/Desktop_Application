...
class RoomClass:
    def __init__(self, home_window):
        ...
        self.hdlbl.config(background=mycolor2)
        self.l1 = Label(self.window, text='Reg_No', background=mycolor, font=myfont1)
        self.l2 = Label(self.window, text='Room Type', background=mycolor, font=myfont1)
        self.l3 = Label(self.window, text='Room no.', background=mycolor, font=myfont1)
        ...
        self.v2 = StringVar()
        self.c2 = Combobox(self.window, values=('101','102','103','104','105','106','107','108'), state='readonly', textvariable=self.v2, font=myfont1,
                           width=18)
        self.v2.set('Room no.')

        # ---------------- table -------------

        self.mytable = Treeview(self.window, columns=['c1', 'c2', 'c3'], height=10)
    ...
    def saveData(self):
        ...
        try:
            ...
            rowcount = self.curr.execute(qry, (
                self.t1.get(),self.v1.get(),self.v2.get()))
            self.conn.commit()
            if rowcount == 1:
    ...
    def deleteData(self):
        ...
        if ans == 'yes':
            try:
                qry = "delete from roominfo_table where Reg_No=%s "
